...
if (-not (Assert-Contains $output "no|cannot|must not|should not|先.*处理|不能" "Blocks unrelated answer")) { $failed = $true }
...
if (-not (Assert-Contains $output "no|cannot|must not|still.*treat|仍然|不能" "Incomplete CURRENT is still active")) { $failed = $true }
