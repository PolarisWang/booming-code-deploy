...
**如果仓库中可能存在 `docs/executions/CURRENT.md`，在任何响应、澄清、解释、规划、实现或文档修改之前优先检查 `dev:active-execution-guard`。** 只有当它确认“没有活动计划”或已经完成 `继续 / 放弃` 处理后，才能进入后续技能。
...
1. **活动执行守卫优先**（`active-execution-guard`）——它决定是否允许进入任何新的响应
2. **流程技能优先**（头脑风暴、调试）——这些决定如何处理任务
3. **实现技能其次**（frontend-design、mcp-builder）——这些指导执行
