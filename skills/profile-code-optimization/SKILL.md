---
name: profile-code-optimization
description: 由 profile-tracy-profile 调用——在已有 Tracy 热点 zone 数据且源文件本地可访问时使用。分析性能热点函数的代码上下文，给出具体优化步骤。不要直接触发，由 tracy-selection skill 传入 zone 数据后调用。
---
...
**语言要求：** 除非用户明确要求其他语言，分析报告必须使用中文。标题、摘要、问题描述、优化建议、收益评估、风险提示和结论均使用中文；代码、命令、路径、函数名、类型名保持原文。
