...
**保存计划到：** `docs/booming/<YYYY-MM-DD-HHmmss>-<feature-name>/plan/<YYYY-MM-DD-HHmmss>-<feature-name>-plan.md`
- （用户的计划位置偏好会覆盖此默认值）
...
**"计划已完成并保存到 `docs/booming/<YYYY-MM-DD-HHmmss>-<feature-name>/plan/<YYYY-MM-DD-HHmmss>-<feature-name>-plan.md`。准备好执行了吗？"**

**执行路径取决于执行环境的能力：**

**如果执行环境有子 Agent（Claude Code 等）：**
- **必需：** 使用 booming-code:subagent-driven-development
- 不要提供选择——子 Agent 驱动是标准方法
- 每个任务使用新鲜的子 Agent + 两阶段审查

**如果执行环境没有子 Agent：**
- 使用 booming-code:executing-plans 在当前会话中执行计划
- 批量执行，设置检查点供审查
