...
| 命令 | 调用方式 | 作用 |
|------|----------|------|
| `filepath` | `python skills/booming-analysis-tracy-selection/tracy_http.py filepath` | 扫描 9090-9099，找到正在运行且已打开 trace 文件的 Tracy UI。端口验证逻辑：请求 `/api/filepath` 并检查返回的 `filepath` 字段非空，避免连接到未加载文件的 Tracy 实例。 |
| `selection` | `python skills/booming-analysis-tracy-selection/tracy_http.py selection` | 读取 Tracy UI 当前高亮选中的 zone 完整数据，包含函数名、线程信息、父子 zone 关系、源文件位置及精确耗时。`valid: false` 表示 Tracy UI 中当前没有选中任何 zone。 |
| `zone <id>` | `python skills/booming-analysis-tracy-selection/tracy_http.py zone 12345` | 按 zone_id 查询单个 zone 详情，返回字段结构与 `selection` 完全相同。用途：递归遍历 `children_ids` 展开调用树，或从父 zone 向下钻取热点。 |
...
```bash
python skills/booming-analysis-tracy-selection/tracy_http.py filepath
```
...
```bash
python skills/booming-analysis-tracy-selection/tracy_http.py selection
```
...
```bash
python skills/booming-analysis-tracy-selection/tracy_http.py zone <zone_id>
```
...
**5 层限制：** 展开达到 5 层后，停止自动展开并提示：
> "已展开 5 层调用树。是否继续展开 [耗时最长的子节点：FunctionName (2.1ms)]？"
...
热点 zone 确定后，检查其源文件是否在本地可访问：

```python
import os
file_path = zone_data.get("file", "")  # 来自 zone JSON 的 file 字段
file_exists = bool(file_path) and os.path.exists(file_path)
```

**文件存在时**：调用 `booming-analysis-code-optimization` skill，将以下上下文传入：
- 热点 zone 的完整 JSON（函数名、文件路径、行号、耗时、子 zone 耗时分布）
- 用户的原始问题描述

**文件不存在时**：告知用户 "源文件 `{file}` 在本地不可访问，跳过代码级分析。"，继续展示热点分析结论。

### 阶段 4：引导下一步

分析结束后主动引导：
- 点击特定子 zone 继续深入
- 如需帧统计或内存分析，提示配合 Tracy MCP 工具
