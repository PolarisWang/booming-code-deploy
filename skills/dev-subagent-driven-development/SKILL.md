...
除非用户明确要求其他语言，控制器写入的 `docs/executions/CURRENT.md`、执行归档、任务摘要，以及通过 `dev:project-wiki-maintenance` 触发的知识文档都必须使用中文。若某个任务会编写设计、计划、wiki、报告或其他 Markdown/文档文件，派发给子 Agent 时必须明确要求该书面产物使用中文；代码、命令、路径、标识符保持原文。
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "更新 CURRENT.md，必要时更新 wiki，并在 TodoWrite 中标记任务完成" [shape=box];
    }

    "读取计划一次，确认任务总数，提取所有任务的完整文本，创建或更新 docs/executions/CURRENT.md，记录上下文，创建 TodoWrite" [shape=box];
    "还有更多任务？" [shape=diamond];
    "为整个实现派发最终代码审查者子 Agent" [shape=box];
    "使用 dev:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];

    "读取计划一次，确认任务总数，提取所有任务的完整文本，创建或更新 docs/executions/CURRENT.md，记录上下文，创建 TodoWrite" -> "派发实现者子 Agent（./implementer-prompt.md）";
    ...
    "代码质量审查者子 Agent 批准？" -> "更新 CURRENT.md，必要时更新 wiki，并在 TodoWrite 中标记任务完成" [label="是"];
    "更新 CURRENT.md，必要时更新 wiki，并在 TodoWrite 中标记任务完成" -> "还有更多任务？";
    "还有更多任务？" -> "派发实现者子 Agent（./implementer-prompt.md）" [label="是"];
    "还有更多任务？" -> "为整个实现派发最终代码审查者子 Agent" [label="否"];
    "为整个实现派发最终代码审查者子 Agent" -> "使用 dev:finishing-a-development-branch";
}
```
...
开始执行前，控制器必须创建或更新 `docs/executions/CURRENT.md`。它是恢复执行的唯一入口，至少应包含：
...
每完成一个任务后，控制器都要更新这份文档。如果任务没有产生长期知识，也要在其中写明“本任务无 wiki 更新”。
...
[确认共有 5 个任务]
[提取所有 5 个任务的完整文本和上下文]
[创建或更新 docs/executions/CURRENT.md：写入设计/计划路径、任务总数、结构化上下文、下一步]
[创建包含所有任务的 TodoWrite]
...
[更新 CURRENT.md：任务进度、最近摘要、错误与纠正、下一步]
[如产生长期知识：使用 project-wiki-maintenance 更新 wiki 和 INDEX]
[标记任务 1 完成]
...
[全部任务完成 + 验证通过 + 必要 wiki 更新完成]
[将 CURRENT.md 归档到 docs/executions/history/...-completed.md]
...
- 让 `docs/executions/CURRENT.md` 落后于当前进度
- 在任务产出长期知识时跳过 wiki 更新
...
**必需的工作流技能：**
- **dev:active-execution-guard** - 必需：开始任何新响应前先处理活动计划
- **dev:using-git-worktrees** - 必需：开始前设置隔离的工作空间
- **dev:writing-plans** - 创建本技能执行的计划
- **dev:project-wiki-maintenance** - 任务后写入长期项目知识
- **dev:requesting-code-review** - 审查者子 Agent 的代码审查模板
- **dev:finishing-a-development-branch** - 所有任务完成后收尾开发工作

**子 Agent 应使用：**
- **dev:test-driven-development** - 子 Agent 对每个任务遵循 TDD

**替代工作流：**
- **dev:executing-plans** - 用于并行会话而非同一会话执行
