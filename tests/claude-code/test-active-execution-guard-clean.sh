...
assert_contains "$output" "continue" "Mentions continue"
assert_contains "$output" "hang\|suspend" "Mentions hang"
assert_contains "$output" "abandon\|abandoned" "Mentions abandon"
assert_not_contains "$output" "complete it\|mark.*complete\|choose.*complete" "Does not offer complete"
...
assert_contains "$output" "no\|cannot\|must not\|should not" "Blocks unrelated answer"
...
assert_contains "$output" "no\|cannot\|must not\|still.*treat\|completeness.*matter\|existence.*means" "Incomplete ACTIVE is still active"
