...
if (-not (Assert-Contains $output "hang|suspend" "Mentions hang")) { $failed = $true }
...
if (-not (Assert-Contains $output "no|cannot|must not|still.*treat|completeness.*matter|existence.*means" "Incomplete ACTIVE is still active")) { $failed = $true }
