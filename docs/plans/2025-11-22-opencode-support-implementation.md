...
> **For agentic workers:** REQUIRED SUB-SKILL: Use booming:executing-plans to implement this plan task-by-task.

**Goal:** Add full booming support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/booming-codex` (lines 40-74)
...
- Reference: `.codex/booming-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'booming' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/booming-codex` (lines 212-280)
 ...
 * (personal skills override booming skills).
 *
 * @param {string} skillName - Name like "booming:brainstorming" or "my-skill"
 * @param {string} boomingDir - Path to booming skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, boomingDir, personalDir) {
    // Strip booming: prefix if present
    const forceBooming = skillName.startsWith('booming:');
    const actualSkillName = forceBooming ? skillName.replace(/^booming:/, '') : skillName;

    // Try personal skills first (unless explicitly booming:)
    if (!forceBooming && personalDir) {
    ...
    // Try booming skills
    if (boomingDir) {
        const boomingPath = path.join(boomingDir, actualSkillName);
        const boomingSkillFile = path.join(boomingPath, 'SKILL.md');
        if (fs.existsSync(boomingSkillFile)) {
            return {
                skillFile: boomingSkillFile,
                sourceType: 'booming',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/booming-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/booming-codex` (add import at top)
...
Run: `node -c .codex/booming-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/booming-codex
...
**Files:**
- Modify: `.codex/booming-codex` (lines 40-74)
...
Run: `.codex/booming-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/booming-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/booming-codex` (lines 16-38, approximately)
...
Run: `.codex/booming-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/booming.js`
...
/**
 * Booming plugin for OpenCode.ai
 *
...
const boomingSkillsDir = path.join(homeDir, '.config/opencode/booming/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const BoomingPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/booming.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/booming.js
...
**Files:**
- Modify: `.opencode/plugin/booming.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "booming:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > booming)
...
Run: `node -c .opencode/plugin/booming.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the booming and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const boomingSkills = skillsCore.findSkillsInDir(
            boomingSkillsDir,
            'booming',
            3
          );
          ...
          const allSkills = [...personalSkills, ...boomingSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install booming skills to ~/.config/opencode/booming/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'booming:';
...
```javascript
    'session.started': async () => {
      // Read using-booming skill content
      const usingBoomingPath = skillsCore.resolveSkillPath(
        'using-booming',
        boomingSkillsDir,
        personalSkillsDir
      );

      let usingBoomingContent = '';
      if (usingBoomingPath) {
        const fullContent = fs.readFileSync(usingBoomingPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingBoomingContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Booming skills: \`booming:skill-name\` (from ~/.config/opencode/booming/skills/)
- Personal skills: \`skill-name\` (from ~/.config/opencode/skills/)
- Personal skills override booming skills when names match
`;

      // Check for updates (non-blocking)
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/booming')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/booming && git pull` to update booming.' :
        '';
...
**Below is the full content of your 'booming:using-booming' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```markdown
# Installing Booming for OpenCode
...
### 1. Install Booming Skills

```bash
# Clone booming skills to OpenCode config directory
mkdir -p ~/.config/opencode/booming
git clone https://github.com/obra/booming.git ~/.config/opencode/booming
```

### 2. Install the Plugin

The plugin is included in the booming repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/booming/.opencode/plugin/booming.js`
...
ln -s ~/.config/opencode/booming/.opencode/plugin/booming.js .opencode/plugin/booming.js
```
...
```
use use_skill tool with skill_name: "booming:brainstorming"
```
...
Personal skills override booming skills with the same name.
...
1. Check plugin file exists: `ls ~/.config/opencode/booming/.opencode/plugin/booming.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/booming/skills`
...
- Report issues: https://github.com/obra/booming/issues
- Documentation: https://github.com/obra/booming
```
...
Booming works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/booming-codex use-skill booming:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/booming-codex bootstrap | head -30`
...
ls -l .opencode/plugin/booming.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/booming.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/booming-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/booming-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/booming.js` created with tools and hooks
