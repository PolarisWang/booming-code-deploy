# Test: project-wiki-maintenance skill (Windows PowerShell)
...
if (-not (Assert-Contains $output "wiki/|project.*knowledge|long-term.*knowledge" "Mentions project wiki scope")) { $failed = $true }
...
if (-not (Assert-Contains $output "docs|executions|kept.*elsewhere|not.*wiki" "Separates docs from wiki")) { $failed = $true }
...
if (-not (Assert-Contains $output "must|required" "Wiki update is required")) { $failed = $true }
...
$output = Run-Claude "How does project-wiki-maintenance decide between module documentation and cross-functional project experience?" 30
if (-not (Assert-Contains $output "module|function" "Mentions module knowledge")) { $failed = $true }
if (-not (Assert-Contains $output "experience|cross-functional" "Mentions project experience")) { $failed = $true }
